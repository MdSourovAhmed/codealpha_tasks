  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="space-y-4">
        {events.map((event) => (
          <div key={event._id} className="bg-white shadow-md rounded-lg p-6 flex justify-between items-center">
            <div>
              <h3 className="text-xl font-semibold">{event.title}</h3>
              <p>{new Date(event.date).toLocaleDateString()}</p>
            </div>
            <div>
              <button onClick={() => handleEdit(event)} className="mr-2 text-blue-500 hover:underline">Edit</button>
